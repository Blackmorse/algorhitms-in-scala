...
trait ImprovedMergerWithComparsion[T] extends Merger[T] {
  this: MergeSort[T] =>
  ...
  protected def merge(arr: Array[T], lo: Int, mid: Int, hi: Int, copyArray: Array[T]): Unit = {
    ...
    if(arr(mid) < arr(mid + 1)) {
      for (i <- lo to actualHi) copyArray(i) = arr(i)
      return
    }

    for (i <- lo to actualHi) {
      if (leftIndex > mid) {
        copyArray(i) = arr(rightIndex); rightIndex += 1
      }
      else if (rightIndex > actualHi) {
        copyArray(i) = arr(leftIndex); leftIndex += 1
      }
